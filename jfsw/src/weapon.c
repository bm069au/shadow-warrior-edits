...
DoRocket(SHORT Weapon)
    ...
    if (TEST(u->Flags, SPR_FIND_PLAYER))
        ...
        VectorMissileSeek(Weapon, 60, 32, 128, 768);
        }
...
SpawnGrenadeExp(SHORT Weapon)
	...
	// Brett edit:
// Player grenades trigger full nuclear explosion.
// Monster grenades remain standard to avoid crashes.
if (OwnerIsPlayer(Weapon))
    return SpawnNuclearExp(Weapon);
        ...
        if (!User[ou->tgt_sp-sprite] ||
            ...
            if ((dist < 18000 && (RANDOM_P2(2048<<5)>>5) < 16) || User[sp->owner]->Counter < 4)
                ...
                if (sectnum != -1 && CanSeePlayer(Weapon))
					...
					//Brett edit change rocket speed
                    //u->jump_speed = -1400;
...
InitBunnyRocket(PLAYERp pp)
	...
	SET(wu->Flags, SPR_FIND_PLAYER);
	wp->pal = wu->spal = 20; // Yellow seeker bunny
